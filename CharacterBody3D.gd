...
func _on_terrain_map_ready():
	...
	# Only spawn dev items if dev mode is enabled
	if DebugManager and DebugManager.is_dev_mode():
		...
		var bplanter = preload("res://src/object/agriculture/bubbleplanter03.tscn").instantiate()
